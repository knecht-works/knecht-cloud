...
export const PUBLISH_FOLLOWUP_PROMPT = [
  ...
  '1. Review the uncommitted changes with `knecht-git status` and `knecht-git diff`.',
  '2. If the run is not on a work branch yet, create one with `knecht-git branch` (a short descriptive name).',
  '3. Commit the changes in logical chunks with proper commit messages via `knecht-git commit`.',
