...
function ddevTree(dir: string): string {
  ...
  return files
    ...
    .join('\n')
    .replace(/^(host_(?:db|webserver|https|mailpit)_port: ).*$/gm, '$1"<port>"')
}

describe('configureSessionEnv', () => {
  it('generated-plain: a repo without anything gets a headless php project with defaults', async () => {
    const dir = repo({ 'README.md': '# lib' })
    const { env, warnings, injected } = await configureSessionEnv(dir, project(), 42, 'env')
  ...
  it('generated-with-versions: detected versions and an override land in config.yaml', async () => {
    const dir = repo({
      'composer.json': '{ "require": { "php": "^8.1" } }',
      '.nvmrc': '20\n',
    })
    const { env } = await configureSessionEnv(dir, project({ phpVersion: '8.2', envVars: [{ key: 'APP_ENV', value: 'dev' }] }), 42, 'env')
    expect(env.phpVersion).toEqual({ value: '8.2', source: 'setting' })
    expect(env.nodeVersion).toEqual({ value: '20', source: '.nvmrc' })
    expect(ddevTree(dir)).toMatchSnapshot()
  })

  it('ddev-tracked: a repo with its own config gets only the overrides, unchanged', async () => {
    const dir = repo({
      '.ddev/config.yaml': 'name: demo\nadditional_hostnames: [alpha]\n',
    })
    const { env } = await configureSessionEnv(dir, project({
      phpVersion: '8.1',
  ...
  it('generated-with-devserver: the daemon and the preview env', async () => {
    const dir = repo({ 'mise.toml': '[tools]\nnode = "22"\n' })
    const { env, devServerPort } = await configureSessionEnv(dir, project({ devServer: 'npm run dev', previewPort: 3000 }), 42, 'env')
    expect(env.previewPort).toEqual({ value: 3000, source: 'setting' })
    expect(devServerPort).toBe(3000)
    expect(ddevTree(dir)).toMatchSnapshot()
  })

  it('generated-with-bun: the repo\'s bun goes into the web image at the pinned version', async () => {
    const dir = repo({ 'package.json': '{ "packageManager": "bun@1.2.3" }', 'bun.lock': '{}' })
    const { env } = await configureSessionEnv(dir, project(), 42, 'env')
    expect(env.packageManager).toEqual({ value: { name: 'bun', version: '1.2.3' }, source: 'package.json' })
    expect(ddevTree(dir)).toMatchSnapshot()
  })

  it('bun without a pinned version installs latest, and the image file goes away when the repo stops using bun', async () => {
    const dir = repo({ 'bun.lock': '{}' })
    await configureSessionEnv(dir, project(), 42, 'env')
    ...
    expect((await configureSessionEnv(dir, project(), 42, 'env')).changed).toBe(false)
    rmSync(join(dir, 'bun.lock'))
    writeFileSync(join(dir, 'pnpm-lock.yaml'), '')
    const { env, changed } = await configureSessionEnv(dir, project(), 42, 'env')
  ...
  it('a repo using pnpm also gets the npm-style store path pnpm 9 and 10 read, other repos do not', async () => {
  ...
  it('a config Knecht generated on an earlier boot is rewritten, not treated as the repo\'s', async () => {
    ...
    await configureSessionEnv(dir, project(), 42, 'env')
    const { env } = await configureSessionEnv(dir, project({ nodeVersion: '18' }), 42, 'env')
    expect(env.source).toBe('generated')
  ...
  it('reports whether the environment definition changed since the last write', async () => {
    const dir = repo({ 'package.json': '{}' })
    expect((await configureSessionEnv(dir, project(), 42, 'env')).changed).toBe(true)
    expect((await configureSessionEnv(dir, project(), 42, 'env')).changed).toBe(false)
    expect((await configureSessionEnv(dir, project({ phpVersion: '8.2' }), 42, 'env')).changed).toBe(true)
    expect((await configureSessionEnv(dir, project({ phpVersion: '8.2', devServer: 'npm run dev', previewPort: 3000 }), 42, 'env')).changed).toBe(true)
    expect((await configureSessionEnv(dir, project({ phpVersion: '8.2', devServer: 'npm run dev', previewPort: 3000 }), 42, 'env')).changed).toBe(false)
    expect((await configureSessionEnv(dir, project({ phpVersion: '8.2' }), 42, 'env')).changed).toBe(true)
  })
})
