...
const workItem = vi.hoisted(() => (id: string) => ({
  ...
  sequenceId: 12,
  name: 'Login broken',
  descriptionHtml: '<p>It fails on <strong>submit</strong>.</p>',
  stateId: 's-todo',
  labelIds: ['l-knecht', 'l-bug'],
  assigneeIds: [],
  createdById: 'user-1',
}))
const api = vi.hoisted(() => ({
  ...
  fetched: {
    id: '77',
    html: '',
    actor: { id: 'user-1', displayName: 'Ann Example' },
...
describe('plane webhook route', () => {
  ...
  it('fires on a created work item with the work item as inputs and object', async () => {
    ...
    const trigger = makePlaneTrigger(project.id, { event: 'created' })
  ...
  it('fires on a label only when the work item gains it', async () => {
    const project = makePlaneProject()
    const trigger = makePlaneTrigger(project.id, { event: 'labeled', label: 'knecht' })
  ...
  it('fires on a work item created with the label, the state or the assignment already set', async () => {
    const project = makePlaneProject()
    const labeled = makePlaneTrigger(project.id, { event: 'labeled', label: 'knecht' })
    const transitioned = makePlaneTrigger(project.id, { event: 'transitioned', state: 'In Progress' })
    const assigned = makePlaneTrigger(project.id, { event: 'assigned' })
  ...
  it('fires on a transition to the configured state', async () => {
    const project = makePlaneProject()
    const trigger = makePlaneTrigger(project.id, { event: 'transitioned', state: 'In Progress' })
  ...
  it('fires when the work item is assigned to the connection account', async () => {
    const project = makePlaneProject()
    const trigger = makePlaneTrigger(project.id, { event: 'assigned' })
  ...
  it('turns a comment mentioning the connection account into a follow-up, finding the project by the work item', async () => {
    ...
    api.fetched = {
      id: '77',
      html: `<p><mention-component entity_identifier="${KNECHT_ACCOUNT}" entity_name="user_mention" label="Knecht"></mention-component> please fix the login</p>`,
      actor: { id: 'user-1', displayName: 'Ann Example' },
      createdAt: '2026-01-01T00:00:00Z',
    }
  ...
  it('accepts the plain @knecht text as a mention', async () => {
    const project = makePlaneProject()
    api.fetched = { id: '78', html: '<p>@knecht have a look</p>', actor: { id: 'user-2', displayName: 'Bob' }, createdAt: '' }
  ...
  it('ignores comments by the connection account and comments without a mention', async () => {
    const project = makePlaneProject()
    api.fetched = { id: '79', html: '<p>@knecht I am Knecht</p>', actor: { id: KNECHT_ACCOUNT, displayName: 'Knecht' }, createdAt: '' }
    expect((await deliver(commented(project, '79'))).json)
      .toMatchObject({ outcome: expect.stringContaining('Knecht itself') })
    api.fetched = { id: '80', html: '<p>just chatting</p>', actor: { id: 'user-1', displayName: 'Ann Example' }, createdAt: '' }
